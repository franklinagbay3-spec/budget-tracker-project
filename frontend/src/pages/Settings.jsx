...
import { MdPerson, MdNotifications, MdSecurity, MdAdd, MdNotificationsActive, MdAccountBalance, MdDelete, MdDownload } from "react-icons/md";
...
function Settings() {
  ...
  const [alerts, setAlerts]       = useState({ overspending: true, billReminders: true, newsletters: false });
  ...
  // FONT SIZE — applied to <html> so everything scales
  useEffect(() => {
                ...
                {tabs.map((tab) => (
                  <button key={tab.id} onClick={() => setActiveTab(tab.id)}
                    className={`w-full flex items-center gap-3 px-4 py-2.5 rounded-xl text-sm font-semibold transition ${
                      activeTab === tab.id ? "bg-[#eff4ff] dark:bg-gray-700 text-[#4648d4]" : "text-gray-500 dark:text-gray-400 hover:bg-[#f8f9ff] dark:hover:bg-gray-700"
                    }`}
                  >
                    {tab.icon} {tab.label}
                  </button>
                    ...
                    <div className="bg-white dark:bg-gray-800 rounded-2xl p-6 border border-[#e5eeff] dark:border-gray-700">
                      ...
                      <div className="mb-6">
                        ...
                        <div className="flex rounded-lg border border-gray-200 dark:border-gray-600 overflow-hidden">
                          <button onClick={() => setDarkMode(false)} className={`flex-1 py-2 text-sm font-semibold transition ${!darkMode ? "bg-[#4648d4] text-white" : "text-gray-500 dark:text-gray-400 hover:bg-[#f8f9ff] dark:hover:bg-gray-700"}`}>
                            ☀ Light
                          </button>
                          <button onClick={() => setDarkMode(true)} className={`flex-1 py-2 text-sm font-semibold transition ${darkMode ? "bg-[#4648d4] text-white" : "text-gray-500 dark:text-gray-400 hover:bg-[#f8f9ff] dark:hover:bg-gray-700"}`}>
                            ☾ Dark
                          </button>
                        </div>
                      </div>
                      ...
                      <div>
                        ...
                        <p className="text-xs text-gray-400 mb-3">Adjust text size across the app</p>
                        ...
                        {/* PREVIEW */}
                        <div className="mt-3 p-3 bg-[#f8f9ff] dark:bg-gray-700 rounded-lg border border-[#e5eeff] dark:border-gray-600">
                          <p className="text-xs text-gray-400 mb-1">Preview</p>
                          <p style={{ fontSize }} className="text-gray-900 dark:text-white font-medium">Budget Tracker — {FONT_SIZES.find(f => f.value === fontSize)?.label} text</p>
                        </div>
                      </div>
                    </div>
                    ...
                    <div className="bg-white dark:bg-gray-800 rounded-2xl p-6 border border-[#e5eeff] dark:border-gray-700">
                      ...
                      <div className="space-y-4">
                        {[
                          ...
                          { key: "newsletters",   label: "Email Newsletters",   desc: "Weekly financial tips" },
                        ].map((item) => (
                          <div key={item.key} className="flex items-center justify-between">
                            ...
                            <button onClick={() => setAlerts(a => ({ ...a, [item.key]: !a[item.key] }))}
                    ...
                    {[
                      ...
                      <div key={item.key} className="flex items-center justify-between p-4 bg-[#f8f9ff] dark:bg-gray-700 rounded-xl">
                        <div className="flex items-center gap-4">
                          <div className="w-10 h-10 bg-white dark:bg-gray-800 rounded-xl flex items-center justify-center shadow-sm">{item.icon}</div>
                          <div>
